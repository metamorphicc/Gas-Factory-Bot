...
- ботом может пользоваться любой, у кого есть ссылка на Telegram-бота
- добавить клиента и создать папку `clients/<client_name>/`
- загрузить файл клиента: `pdf`, `doc`, `docx`, `jpg`, `png`
- после загрузки присвоить файлу нормальное имя; для нескольких файлов - по одному имени на строку
- переименовать уже сохраненный файл через кнопку `Переименовать файл`
- вести Excel-реестр клиентов в `<STORAGE_ROOT>/clients.xlsx`
- получить Excel-реестр в чате через кнопку `Таблица клиентов`
...
Нажмите `Переименовать файл`, выберите клиента, выберите файл и отправьте новое имя одним сообщением.
Расширение сохраняется автоматически.

## Таблица клиентов

Нажмите `Таблица клиентов`, и бот отправит текущий Excel-файл `<STORAGE_ROOT>/clients.xlsx` в чат.
...
- `<STORAGE_ROOT>/clients.xlsx` - Excel-реестр клиентов и пробных данных

Столбцы в Excel-реестре:
...
Это MVP demo для клиента, не production IAM. Ботом может пользоваться любой, у кого есть ссылка на Telegram-бота. Файлы ограничены по типу и размеру, имена папок и файлов очищаются перед сохранением.
